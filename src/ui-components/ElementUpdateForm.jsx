...
import { getElement, getQueue, listQueues } from "../graphql/queries";
...
export default function ElementUpdateForm(props) {
  const {
    id: idProp,
    element: elementModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = elementRecord
      ? { ...initialValues, ...elementRecord, queueID }
      : initialValues;
    setName(cleanValues.name);
    setPhone(cleanValues.phone);
    setEmail(cleanValues.email);
    setPositiion(cleanValues.positiion);
    setLink(cleanValues.link);
    setQueueID(cleanValues.queueID);
    setCurrentQueueIDValue(undefined);
    setCurrentQueueIDDisplayValue("");
    setErrors({});
  };
  const [elementRecord, setElementRecord] = React.useState(elementModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getElement.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getElement
        : elementModelProp;
      const queueIDRecord = record ? record.queueID : undefined;
      const queueRecord = queueIDRecord
        ? (
            await client.graphql({
              query: getQueue.replaceAll("__typename", ""),
              variables: { id: queueIDRecord },
            })
          )?.data?.getQueue
        : undefined;
      setQueueID(queueIDRecord);
      setSelectedQueueIDRecords([queueRecord]);
      setElementRecord(record);
    };
    queryData();
  }, [idProp, elementModelProp]);
  React.useEffect(resetStateValues, [elementRecord, queueID]);
  ...
  return (
    <Grid
      ...
      {...getOverrideProps(overrides, "ElementUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || elementModelProp)}
